...
### 断点续训跳过训过的内容
- 首先保存的文件夹命名方式是：`checkpoint_folder = f"{PREFIX_CHECKPOINT_DIR}-{self.state.global_step}"`，不过读取已训步长不是根据这个
- 确定已经训过多少epoch，以及当前epoch已经完成了多少个step是根据 Trainer.state变量来记录的
    - self.state中有一个关键的参数：global_step，在训练过程中会做更新`self.state.global_step += 1`
    - 保存的时候同时会保存state：`self.state.save_to_json(os.path.join(staging_output_dir, TRAINER_STATE_NAME))`
    - 训练的时候会根据state确定从多少步开始。
        ```python
