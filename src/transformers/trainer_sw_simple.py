...
class Trainer:
    ...
    def train(
        ...
        **kwargs,
        ...
        if (args.fp16_full_eval or args.bf16_full_eval) and not args.do_train:
            self._move_model_to_device(self.model, args.device)  #TODO
        ...
        if (
            ...
            and not self.is_fsdp_enabled  #TODO 断点续训加载模型的时候，为什么不能是分布式
        ):
            self._load_from_checkpoint(resume_from_checkpoint)
            # In case of repeating the find_executable_batch_size, set `self._train_batch_size` properly
            state = TrainerState.load_from_json(os.path.join(resume_from_checkpoint, TRAINER_STATE_NAME))
            if state.train_batch_size is not None:
                self._train_batch_size = state.train_batch_size #[ ]如果是断点续训，会使用原来的 train_batch_size
    ...
    def _inner_training_loop(
        ...
        if has_length(train_dataloader):
            ...
            if args.max_steps > 0:
                ...
                max_steps = math.ceil(args.num_train_epochs * num_update_steps_per_epoch) #实际上是num_update_steps
        ...
        if not delay_optimizer_creation:
            self.create_optimizer_and_scheduler(num_training_steps=max_steps) #TODO 这里会跟is_deepspeed_enabled冲突吗
        ...
        if use_accelerator_prepare:
            ...
            if hasattr(self.lr_scheduler, "step"):
                if self.use_apex:
                    ...
                    model, self.optimizer = self.accelerator.prepare(self.model, self.optimizer) #[ ] 貌似这里会让deepspeed config来控制scheduler
            else:
        ...
        if resume_from_checkpoint is not None and os.path.isfile(os.path.join(resume_from_checkpoint, TRAINER_STATE_NAME)):
            ...
            if not args.ignore_data_skip:  #默认情况下会执行这里的skip
                steps_trained_in_current_epoch = self.state.global_step % (num_update_steps_per_epoch)
                steps_trained_in_current_epoch *= args.gradient_accumulation_steps #[ ] 换算成实际要跳过多少个batch
            else:
                steps_trained_in_current_epoch = 0
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            if epoch == epochs_trained and resume_from_checkpoint is not None and steps_trained_in_current_epoch == 0:
                self._load_rng_state(resume_from_checkpoint) #TODO
            ...
            for step, inputs in enumerate(epoch_iterator): #[ ]这里内部有很多冗余判断
                ...
                if step % args.gradient_accumulation_steps == 0:
                    self.control = self.callback_handler.on_step_begin(args, self.state, self.control) #[ ]指的是update_step

                with self.accelerator.accumulate(model): #[ ] 应用梯度累积
                ...
                if (
                    ...
                    if is_last_step_and_steps_less_than_grad_acc:
                        self.accelerator.gradient_state._set_sync_gradients(True) #[ ] 如果不够grad_acc数量，直接手动同步梯度
    ...
    def evaluation_loop(
        ...
        self.callback_handler.eval_dataloader = dataloader  #[ ] TODO
